	...
	for ff in annotation_csv_files:
		...
		while k <= (len(tmp2) + 1 - 2 * window_size):
			...
			for j in range(window_size):
				if y_pre[j] == "go_sleep_": #modified
					go_sleep += 1
				elif y_pre[j] == "stand": #modified
					stand += 1
				elif y_pre[j] == "pass_out_": #modified
					pass_out += 1
				elif y_pre[j] == "just_lay_": #modified
					just_lay += 1
				elif y_pre[j] == "stn": #modified - stn might need to be obmitted in the future
					stn += 1

			if go_sleep > window_size * threshold / 100: #modified
				y[k // slide_size, :] = np.array([0, 1, 0, 0, 0, 0])
			elif stand > window_size * threshold / 100: #modified
				y[k // slide_size, :] = np.array([0, 0, 1, 0, 0, 0])
			elif pass_out > window_size * threshold / 100: #modified
				y[k // slide_size, :] = np.array([0, 0, 0, 1, 0, 0])
			elif just_lay > window_size * threshold / 100: #modified
...
if not os.path.exists("input_files/"):
	...
	# print("../")
for i, label in enumerate (["go_sleep_", "stand", "pass_out_", "just_lay_", "stn"]): #modified - stn might need to be obmitted in the future
	filepath1 = "C:\MyPaperWork_YH\\data\\netlink\\" + str(label)  + "*.csv"
	filepath2 = "C:\MyPaperWork_YH\\data\\netlink\\annotation_" + str(label)  + "*.csv"
