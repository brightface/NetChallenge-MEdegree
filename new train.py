...
#data import
#x_go_sleep, x_stand, x_pickstn, x_run, x_just_lay, x_standstn, x_go_sleep, \
#y_go_sleep, y_stand, y_pickstn, y_run, y_just_lay, y_standstn, y_go_sleep = csv_import()
x_go_sleep, x_pass_out, x_just_lay, y_go_sleep, y_pass_out, y_just_lay = csv_import()

#print(" go_sleep =",len(x_go_sleep), " stand=", len(x_stand), " pickstn =", len(x_pickstn), " run=", len(x_run), " just_lay=", len(x_just_lay), " standstn=", len(x_standstn), " go_sleep=", len(x_go_sleep))
print(" go_sleep =",len(x_go_sleep), " pass_out=", len(x_pass_out), " just_lay=", len(x_just_lay))
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    for i in range(kk):
        ...
        wifi_x_train = np.r_[x_go_sleep[int(len(x_go_sleep) // kk):],x_pass_out[int(len(x_pass_out) // kk):], x_just_lay[int(len(x_just_lay) // kk):]]

        wifi_y_train = np.r_[y_go_sleep[int(len(y_go_sleep) // kk):],  y_pass_out[int(len(y_pass_out) // kk):], y_just_lay[int(len(y_just_lay) // kk):]]

        wifi_y_train = wifi_y_train[:,1:]

        wifi_x_validation = np.r_[x_go_sleep[:int(len(x_go_sleep) // kk)], x_pass_out[:int(len(x_pass_out) // kk)], x_just_lay[:int(len(x_just_lay) // kk)]]

        wifi_y_validation = np.r_[y_go_sleep[:int(len(y_go_sleep) // kk)],  y_pass_out[:int(len(y_pass_out) // kk)], y_just_lay[:int(len(y_just_lay) // kk)]]
